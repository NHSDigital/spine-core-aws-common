...
from spine_core_aws_common.log.loglineprocessor import (check_for_param_dictionary,
                                  LogLineProcessor)
...
def can_encode_string(value):
    """
    Python 3 compatible check to see if value is either a base or
    unicode string.
    """
...
def evaluate_log_keys(log_details, log_row_dict):
    ...
    for log_key in log_row_dict:
        ...
        log_details.audit_log_required = audit or \
            log_details.audit_log_required
...
def create_placehold_log(log_row_dict):
    ...
    """
    log_text = ''
    for identifier in lc.IDENTIFIERS:
