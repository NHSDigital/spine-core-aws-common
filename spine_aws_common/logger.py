...
from spine_aws_common.log.formatting import add_default_keys, evaluate_log_keys, create_placehold_log, create_log_line,\
...
class Logger:
    ...
    def writeLog(self, logReference='LAMBDAUTI9999', errorList=None,
        ...
        log_details = get_log_details(
            logReference, self._logBaseDict, self._logBaseCache, pythonlogging=False)
        ...
        logPreamble = self._createLogPreamble(
            timeNow, log_details.log_level, processName, logReference)
    ...
    def _createLogPreamble(self, timeNow, logLevel, processName, logReference):
        ...
        logTimestampString += str(int(timeNow.microsecond /
                                  1000)).rjust(3, '0')
    ...
    def _writeToCloudWatch(self, timeNow, logPreamble, logText, substitutionDict, logType, errorList=None):
        ...
        logLine = create_log_line(
            logPreamble + ' Log_Index=' + logIndex, logText, substitutionDict)
        ...
        if logType == lc.LFR_CRASHDUMP and errorList and len(errorList) >= 3:
            traceback.print_exception(
                errorList[0], errorList[1], errorList[2], None)
