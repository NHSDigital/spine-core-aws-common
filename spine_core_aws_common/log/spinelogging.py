...
def get_log_base_config(log_base):
    ...
    if not log_base_dict:
        ...
        for log_ref in log_base_config.sections():
            try:
                level = log_base_config.get(log_ref, lc.SECTION_LEVEL)
            except ConfigParser.NoOptionError:
                level = 'INFO'

            try:
                text = log_base_config.get(log_ref, lc.SECTION_TEXT)
            except ConfigParser.NoOptionError:
                text = log_base_config.get('IGUTI9999', 'Log Text')
...
class SpineLogger(logging.Logger):
    ...
    def get_process_name(cls):
        ...
        return logging.getLogger(lc.SPINE_LOGGER).process_name

    @classmethod
    def get_log_permission(cls):
        """ Return the current log permission """
        return logging.getLogger(lc.SPINE_LOGGER).log_permission

    @classmethod
    def get_log_base_dict(cls):
        """ Return the current log base dict """
        return logging.getLogger(lc.SPINE_LOGGER).log_base_dict

    @classmethod
    def get_log_base_cache(cls):
        """ Return the current log base cache """
        return logging.getLogger(lc.SPINE_LOGGER).log_base_cache

    @classmethod
    def get_severity_threshold(cls):
        """ Return the current severity threshold """
        return logging.getLogger(lc.SPINE_LOGGER).severity_threshold
